...
public:
  ...
  void setupVideo() {
    ...
    void print_detection(vector<AprilTags::TagDetection> detections, WebSocket::pointer wss) const {
        string objects = "[";
    ...
    Eigen::Matrix4d matrix = detection.getRelativeTransform(m_tagSize,m_fx, m_fy, m_px, m_py);
        
    detection.getRelativeTranslationRotation(m_tagSize, m_fx, m_fy, m_px, m_py,
                                             translation, rotation);
    matrix.transposeInPlace();
    Eigen::Matrix4d matrix2 = matrix;
    string matArray = "[";

    for(int col = 0; col<16; col++) {
        if(col == 15)
        {
            matArray = matArray + std::to_string(matrix(col)) + "]";
        }
        else {
            matArray = matArray + std::to_string(matrix(col)) + ",";
        }
    ...
    void processImage(cv::Mat& image, cv::Mat& image_gray, WebSocket::pointer wss) {
  ...
  // Load and process a single image
    void loadImages(WebSocket::pointer wss) {
    cv::Mat image;
    cv::Mat image_gray;
  ...
  // and information about detections generated
    void loop(WebSocket::pointer wss) {
...
int main(int argc, char* argv[]) {
  Demo demo;
//  wss.reset(WebSocket::from_url("ws://localhost:8080"));
